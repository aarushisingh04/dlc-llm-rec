...
def train(
    # model/data params
    base_model: str = "",  # the only requ ired argument
    ...
    for path in val_data_path:
        if path.endswith(".json"):
            ...
            val_data_list.append(load_dataset(path)) # ['instruction', 'input', 'output']
    ...
    for i in range(len(val_data_list)):
        val_data_list[i] = val_data_list[i].map(lambda x: generate_and_tokenize_prompt(x)) # ['instruction', 'input', 'output', 'input_ids', 'attention_mask', 'labels']
